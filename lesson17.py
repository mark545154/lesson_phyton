...
def sum_od(n, rand_list):  # передаём загаданное число и кол-во игроков
    ...
    for num in rand_list:  # Перебираем кол-во игроков
        x = num % 10  # Проверяем делится ли число
        y = num // 10  # Находим остаток от деления
        if (x + y) % n == 0:  # Делится ли число на загаданное
            priz_num.append(num)  # Добавляем призовые номера в конец списка
    return priz_num  # Выводим результат


print(num)  # Загаданное число
print(random_list)  # Смотрим числа, которые делятся без остатка
print(sum_od(num, random_list))  # Загаданное число с помощью random и Кол-во игроков
